...
namespace StubbornBrowser
{
    public partial class TabView : UserControl, IDisplayHandler, IDownloadHandler, ILifeSpanHandler, IContextMenuHandler
    {
        ...
        public TabView(MainWindow mw, string url)
        {
            ...
            splitChar = (char) 42;
            WebView.DownloadHandler = new StubbornBrowser.DownloadHandler();//处理下载
        ...
        private async void WebView_FrameLoadStart(object sender, FrameLoadStartEventArgs e)
        {
            await Dispatcher.BeginInvoke((Action) (() =>
            {
                try
                {
                    ...
                    bdLoading.Visibility = Visibility.Visible;
                    tbLoading.Text = "正在加载" + e.Url;
                }
                catch (Exception ex)
                {
            ...
            if (e.Key == Key.Enter)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        if (Convert.ToString(dyn.SE) == "Baidu")
                        {
                            textBox.Text = textBox.Text.Replace(textBox.Text, "http://www.baidu.com/s?wd=" + textBox.Text);
                            WebView.Load(textBox.Text);
                        }
                    }
                    catch (Exception ex)
        ...
        public bool OnTooltipChanged(IWebBrowser chromiumWebBrowser, ref string text) { return false; }
        public void OnLoadingProgressChange(IWebBrowser chromiumWebBrowser, IBrowser browser, double progress) { }
        public bool OnAutoResize(IWebBrowser chromiumWebBrowser, IBrowser browser, CefSharp.Structs.Size newSize) { return true; }
